...
Three observations follow. First, top-1 test accuracy (0.630 and 0.735) falls below the validation accuracy shown in Table~\ref{tab:classification_results} (0.704 and 0.782) and Figure~\ref{fig:val_vs_train_accuracy} for both networks, by roughly seven and five percentage points respectively. Since the checkpoint was selected on the validation split, some optimism there is expected; the test figures are the honest estimate of generalization. The ranking of the two architectures is unchanged.
...
\midrule
ResNet-18 & 5 & 512 & 70.4\% & $\sim$192 min \\
...
\centering
\includegraphics[width=\linewidth]{resnet_val_training_accuracy.png}
\caption{ResNet-18 (validation vs training accuracy, 5-eopchs)}
\label{fig:val_vs_train_accuracy resnet}
\end{subfigure}
\hfill
\begin{subfigure}{0.49\textwidth}
\centering
\includegraphics[width=\linewidth]{efficientnetB0_training_val_accuracy_10epochs.png}
\caption{EfficientNet-B0 (validation vs training accuracy, 10-epochs)}
\label{fig:val_vs_train_accuracy effnet}
\end{subfigure}
\caption{Validation and Traning Accuracy change for each epoch while fine-tuning the 5-epocs for ResNet-18 and 10-epochs for EfficientNet-B0.}
\label{fig:val_vs_train_accuracy}
\end{figure}
